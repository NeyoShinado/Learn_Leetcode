...
②无后效性：每个状态都是过去决策的总结，不会影响当前和未来的决策；
...
### 背包问题
# ------------------背包九讲------------------ #
完全背包、多重背包都能转化为01背包问题解决。核心思想是将重复的元素划分为对应范围的连续2次幂个基本单元构成的“新物体”。连续的k组成的基本元件数组：2^k能不重复元素地构成任何整数。这样能将原本O(N)的遍历复杂度优化为O(log(V/Ci))个基元的遍历复杂度。
