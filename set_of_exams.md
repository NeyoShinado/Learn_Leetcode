...
### python 多维数组排序
首先注意自带的sort()方法原地修改，没有返回值；sorted()函数才有返回值；
要指定排序的维度或规则可以使用sort()的key参数，插入匿名函数就能生成元素排序的键值。
如按字典的value排序：
>res = sorted(d.items(), key=lambda x:x[1])
给字符串排序，要求小写字母、大写字母、奇数、偶数的顺序排列：
这里的思路是用关键字构造一个元组，元组的hash会被直接用来比较，越靠前的元素导致的hash增量越大。
>res = sorted(s, key=lambda x: (x.isdigit(), x.isdigit()andint(x)%2==0, x.isupper(), x.islower(), x))
...
### python字符串
1.*python join()在长字符串拼接时似乎会出错，原因未明，建议用+
2.多分割符的字符串分割
①循环使用分隔符处理：
for i in seqSet:
    s = "".join(s.split(i))
②使用正则表达式
s.split("[seqs]+")    # +代表[分隔符集]中的至少一个
...
④优化状态遍历方式，节省空间；
⑤进一步压缩状态，优化算法复杂度；
**注：动态规划中有一种状态设法--dp[k]至多进行了k次XX的YY或前K种XX所构成的最佳YY
上述设法在背包问题和股票问题中都出现过，主要有两点优势：①限定物品的出场顺序，通过这个“隐形的”顺序模拟出组合的效果；②dp[k]包含了dp[k-1]的情况，基于dp[k-1]建立更优解，直接维护结果，可以省略更新完成后再一次遍历搜索最优解。
...
>Summary:
1.遍历过程中同步更新历史最低位，就能实时更新当天的最大利润；或逆向更新历史最高位，思路一样。
...
>Summary:
1.DP中另需注意隐含条件--是否持有股票，以及关于“至多交易k次”的初始条件设置


(188)max_profitⅣ
最多允许K次买卖，求最大利润
>Summary:
1.跟Ⅲ类似，期限k根据N//2确定。且第i天只跟i-1天有关，空间复杂度可以优化为O(k)，但k要逆序更新以保持临时变量。
...
*(1000)合并石堆
1.有N堆石头，第i堆有a[i]块。每次可合并连续的K堆石为1堆，移动成本为这K堆石的总数。问所有石头合并为1堆的最低成本。
...
### Greedy/贪心算法
贪心算法跟动态规划很相似，不过动态规划的状态转移是由多个状态确定的；而贪心算法则是直接选取当前最优的方案(局部最优)，剔除其他的方案。贪心的最优结果就是由每个子问题的最优结果组成的，不用回退；动态规划则根据以前的结果进行选择，有回退功能。
...
DFS遍历使用全局变量和局部变量进行递归的区别：①使用局部变量方便维护，但内存消耗O(N^2)增加；
②全局变量节省空间和时间，但每一层递归需要回溯，跟踪变化。
...
1210.带旋转的迷宫穿越
占两单位长、向右的蛇有向右移、向下移、向右转和向左转四种行动状态，要求扫过的位置都为0。问最少多少步到达出口(n-1,n-2),(n-1,n-1)


### 前缀和
前缀和很适合有序数据(无环链表，数组，树)的路径和和差分问题，而且速度往往很快。其应用场景一般都是有覆盖范围、边界梯度等特征出现。
(437)path_sum
...
**零、注意跳台阶问题和完全背包问题十分相似
跳台阶问题求的是解决方案的排列数，而背包问题求的是解决方案的组合数。两个问题的核心代码和状态转移方程基本一样，唯一区别就是变量遍历的顺序不同，具体见代码：
假设背包容量/台阶数为V，物品体积/跳跃阶数为Ci
#(1)跳台阶排列问题
# 跳台阶的状态dp[i]设为跳到状态i的方案数(排列数)，转移方程为dp[i]=Σjdp[i-C[j]]，不同台阶方案数的关于上一个台阶方案的累积过程就已经考虑了跳法的顺序了。
def Stage(V, C):
    N = len(C)
    dp = [0 for _ in range(N+1)]
    dp[0] = 1
    for v in range(1,V+1):  #第一层台阶的遍历
        for i in range(N):  #跳法的遍历
            if v < C[i]:
                continue
            dp[v] += dp[j-C[i]]  #状态空间可以进一步简化为单个变量，为了方便讨论继续使用数组
    return dp[-1]
#(2)完全背包组合问题
# 完全背包求的是组合问题，子问题要复杂些。状态dp[i][j]设为只用前i种物品消耗j体积的方案数(代码使用1维数组是优化后的版本，问题的本质还是二维的)，为什么要考虑物品种类维度i？为什么要设得这么别扭呢？原因是遍历时体积v的方案数没有一个累加过程，一次只添加一种选择，每次添加新物品关注的只是凑成体积v的组数，关注的是物品的数量组合。所以求出来的是组合数。
def multiPackage():
    N = len(C)
    dp = [0 for _ in range(N+1)]
    dp[0] = 1
    for i in range(N):  #第一层物品种类的遍历
        for v in range(1, V+1):  #第二层背包容量的遍历
            if v < C[i]:
                continue
            dp[v] += dp[j-C[i]]  #一维状态是空间优化后的结果，本质还是关于二维变量的问题
    return dp[-1]
...
②**状态F[i,v]代表前i种物体(不一定都选)花费体积v的最大价值，“使用前i种物体”的设法是为了在最新状态维护最佳结果，无需重新遍历搜索；**
...
②伪代码：
#*同组物品互斥，所以要最后一轮循环中比较取舍，不能放在v遍历之前(否则会得到同组物品)
for k = 1:K:
    for v = V:0:
...
##4## 无向无环图的最少步数遍历
核心思想：从起点出发的所有路径都走两遍，除了最长的一条只走一遍
