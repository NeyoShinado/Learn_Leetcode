...
for i = 1:N
    ...
    # 而完全背包中恰需要从可能选了i的状态F[i,v-Ci]中递推，所以必须使用顺序
        F[v] = max{F[v], F[v-Ci]+Wi}


三、多重背包问题
背景：不同点在于第i种物品最多Mi件可用，仅相当于改变了件数范围
①思路：和完全背包类似，状态转移方程F[i, v] = max{F[i-1, v-k*Ci]+k*Wi|0≤k≤Mi}，总体复杂度为O(NVΣMi)
...
①思路：一个主件附件套装能有2^n+1种选法：一个不选、选主件、选第一个附件...。鉴于这些选择策略是互斥的(一次最选其中最佳的一种)。主件和附件集合能看作一个物品组。
