...
设计方向：
①自顶向下：递归+记忆化；
②自底向上：(递推)从最小问题开始根据最优子结构逐步解决大规模问题。
...
# ---------------------------------------------
>Summary:
1.构建查询的一般思路：①优先考虑单表，group by,order by,limit等效率也比多表高；②其次考虑用连接，小表驱动大表+建立合适索引+合理运用连接条件能解决大部分问题。但join级数不宜过多；③子查询、笛卡尔积尽量不要使用，因为每条记录都要操作一遍，效率过低；④自定义变量在复杂SQL中用处很大；⑤带聚合功能的查询需求应用窗口函数往往是最优选择。
