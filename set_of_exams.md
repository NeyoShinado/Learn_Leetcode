...
>Summary:
1.单调栈/哨兵技巧：可知围城的矩形符合木桶效应。从左往右遍历所有可能矩形需要O(N^2)，都有遇到高度严格小于当前矩形左边柱形高度时，当前矩形的最大面积才确定。高的左边界矩形先结束更新，有先进先出的规律，可以维护一个栈记录柱形的坐标，弹出时能同时获取宽度和高度更新面积。栈内的柱形高度小的一定最后弹出，高度是递增的，故称为单调栈。
