...
def Stage(V, C):
    ...
    return dp[-1]
#(2)完全背包组合问题
# 完全背包求的是组合问题，子问题要复杂些。状态dp[i][j]设为只用前i种物品消耗j体积的方案数(代码使用1维数组是优化后的版本，问题的本质还是二维的)，为什么要考虑物品种类维度i？为什么要设得这么别扭呢？原因是遍历时体积v的方案数没有一个累加过程，一次只更新一次，每次添加新物品关注的只是凑成体积v的组数，关注的是物品的数量组合。所以求出来的是组合数。
def multiPackage():
    N = len(C)
    dp = [0 for _ in range(N+1)]
    dp[0] = 1
