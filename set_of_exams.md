...
## 1.二分查找
应用背景：二分查找用于查询一个将区间分为两份的边界值，循环中间不断排除一半元素，把目标元素放在最后判断；
实现细节：两者都一样，通过二分整除求得中值mid=(l+r)//2，然后nums[mid]与x比较判定缩小那一边边界，x划分的左区间较大(nums[mid]<=x)则收缩左边界l=mid+1，否则收缩右边界r=mid-1(±1的目的在于最后打破l≤r的约束，得到结果)；唯一不同的地方是最后边界loc的更新方式，若要求左边界，则要求nums[loc]一直在x左侧，nums[mid]<=x时才更新loc：loc=mid，否则反之。
