...
重要概念：
①重复子问题：问题的解需要重复计算大量重复sub-问题的状态/解。所以需要空间换时间，记录子问题的最优解；
②最优子结构：无论问题规模如何变化，较大规模的问题的解可由较小的子问题最优解获得。对应状态转移方程；
③无后效性：每个状态都是过去决策的总结，不影响当前和未来的决策也不受其影响；
...
①直接设计为问题目标；
②设计为有利于状态转移方程的中间变量；
③*根据问题调整状态结构，以致程序逻辑便于维护(152)；

设计方向：
①自顶向下：递归+记忆化(139)；
②自底向上：(递推)从最小问题开始根据最优子结构逐步解决大规模问题(一般为填表格的形式，如背包问题中的表格复用)。


(139)word_break
给定一非空字符串s和包含非空无重复单词列表的字典wordDict，判定s是否可以拆分为一个或多个在字典出现的单词。(字典的单词可以重复使用)
>Summary:
1.一次划分后剩余的子字符串是同样的分词子问题，所以可以判定这是动态规划问题；
2.一种方法是使用boolean数组记录id位置上能否成功分词，这种办法需要双重循环递归；
3.另一种方法是根据问题的递归结构使用DFS+记忆化，优化回溯后步骤的冗余运算，使复杂度降为O(N)。


(152)max_product/最大乘积
给定一整数数组nums，返回最大乘积的连续子数组。
>Summary:
1.!状态设为以nums[i]结尾的最大乘积连续数组，这样做的原因是便于维护"连续数组"这一条件；若直接采用"nums[0:i]中最连续数组乘积"的设法，状态的维护将多了决定结尾元素的复杂讨论。
2.由于数组有正负值，每个状态应该记录当前的最大值及最小值，以解决nums[i]为负数时翻转的问题；
...
!(1143)LongestCommonSubsequence / 最长公共子序列
给定两个字符串/数组，返回这两个字符串/数组的最长公共子序列的长度。
>Summary:
1.首先对问题类型简单分析，子序列是排列类的问题；而最大长度一般是排列的剪枝优化，再考虑两个字符串/数组，就是二维的动态规划问题了。
2.状态表dp[i][j]设为字符串s1[0:i]和s2[0:j]的最长公共子序列长度。状态转移方程：当s1[i]==s2[j]时，dp[i][j]=dp[i-1][j-1]+1；当s1[i]≠s2[j]时，dp[i][j]=min(dp[i-1][j], dp[i][j-1])
...
*(321)maxNumber
给定长度m、n两个数组，从这两个数组中取出k个数字拼接一个新的数，要求同一数组中取出的数保持原来的相对顺序。问满足条件的最大数。
...
>Summary:
1.一个元素出现一次+其余出现两次，直接位操作a^b^b累积异或，最后剩下的就是答案。


(365)canMeasureWater
两个水杯取水，容量分别为x和y，问能否取到体积为z的水？
>Summary:
1.采用贝祖定理，水加满或倒掉都是没意义的(和空杯装满水状态一样)。注意两水杯永远没有都是不满且有水的状态。所以只能是一个水杯给另一个加水能凑出其他组合。
最终可以认为每次操作只会给水的总量带来x或y的变化，能凑成的z=ax+by(a、b可正可负)，该方程有解当且仅当z为x，y最大公约数的整数倍。违反实际的情况由边界条件筛选掉x+y≥z。
2.另一种方法是采用带重复状态检测剪枝的BFS分操作讨论。


(商汤T2)水杯问题加强版
同样的操作，给定四个容量杯子Si，以及最终杯子的液体体积Ti，问最少多少步操作达到，不能则返回-1。
>Summary:
1.N个杯子的倒水问题，尤其是区分杯子状态的倒水问题，用不了贝祖算法，没有特别高效的方法。每轮有N(N+1)种选择，一般采用BFS+关键字状态哈希剪枝查找最短操作数。
...
(372)*SuperPower
计算a^b对正整数c的模，a是一正整数，b是以数组形式给出的大正整数。
>Summary:
1.该题难点--①如何处理大整数b；②如何计算复杂幂；③优化幂运算；
2.解决方式--①大整数可以通过简单幂累乘的方式递归解决；②乘法项为避免溢出应使用(A*B)%C=(A%C)(B%C)%C的技巧。；③幂运算的累乘可以通过分治思想实现对数级的优化。
!3.奇偶判断语句if(x&1==1)中，&会被视为AND操作符，因为与运算优先级不如==高，会报错int combined with boolean.
*4.大数求模问题可以通过欧拉公式来降幂优化，比常规迭代方法在常数级上有优势。
...
查找字符串s中的最长子串，满足子串内的某些字符只出现偶数次，返回其长度。
>Summary:
1.首先使用前缀和减少子串的重复统计；
2.偶数子串要求遍历前方下标的所有前缀和，可以通过直接维护下标i的奇偶性而非和来实现状态压缩，只要i,j位置的奇偶状态相同，说明[i,j-1]子串符合要求。同时将状态压缩在2^M内；
3.要最快找到最长子串，只需要使用哈希表记录第一次出现的状态下标即可。


## 编码
(腾讯T5)给定一编码范围a~y的25字母，1-4位的编码按顺序写为a,aa,aaa,aaaa,aaab,...,b,ba,baa,...序号从0开始，求任意输入编码对应的序号。
...
### Permutation/排列组合
>Summary:
1.排列组合的遍历通常要枚举，优化的思路往往是提取限制条件提前剪枝以降低枚举的复杂度。
...
(282)*addOperators/给表达式添加符号⭐
给定一整数字符串和目标值，在字符串之间插入+、-或*实现运算，返回所有可能的表达式。
>Summary:
1.题目涉及多个子问题：①子元素的划分；②前导零的排除；③乘法优先级的实现。以下一一解答；
2.字符串的表达式构造可看成四种操作符"+","-","*",""的插入问题。空字符串实现多位数划分的功能；
3.分割的数字直接带上操作符添加到结果尾部，"*"的优先级可通过回溯实现（注意乘法递归以cur*pre放到pre中，以实现累乘）：
pre:a, cur:b -> a+b -- val = a+b;
pre:b, cur:c -> a+b*c -- val = val-pre+cur*pre
4.递归函数分为子数扩展（插入""）和子数运算（插入运算符）两部分，cur遇到前导0直接跳过扩展部分，进入运算符插入。
...
#282添加三种符号
1.递归能实现指数级别的遍历
2.*背包算法，这题有元素、目标值等背包问题的特征，但需要转化才能用背包算法解决。
因为涉及到操作枚举的场景，每个元素的不同符号选择会衍生出不同的dp暂态，不能合并讨论。
而通过正符号元素x、负符号元素y和为Sum，差为S，提取出x=(Sum+S)/2的约束条件。这样原问题就从指数级问题转化为能用背包解决的组合选择问题。
...
查找数组的第一个缺失自然数。要求TC:O(N),SC:O(1)


(84)!largers_rectangle_area
给定n个非负整数，表示并排的柱形高度，柱子宽度为1，求该柱形图中最大的矩形面积。
3.类似的单调栈问题42、739、496、316、901、402、581
>Summary:
1.单调栈/哨兵技巧：可知围成的矩形符合木桶效应。从左往右遍历所有可能矩形需要O(N^2)，都有遇到高度严格小于当前矩形左边柱形高度时，当前矩形的最大面积才确定。高的左边界矩形先结束更新，有先进先出的规律，可以维护一个栈记录柱形的坐标，弹出时能同时获取宽度和高度更新面积。栈内的柱形高度小的一定最后弹出，高度是递增的，故称为单调栈。
另需要注意特殊情况：①最矮柱形在中部，导致有弹出时栈为空的现象；②以及高柱形在右端，导致遍历完栈非空的情况。
为此在两端加上高度为“0”的两个柱形，保证维护正常，两边的柱形称为哨兵。
...
#* 注意区分连续序列和连续数组
>Summary:
1.问题通过两个步骤解决：①构建快速查询结构(哈希表)②遍历数组元素，查询对应起始元素连续序列的最大长度；使用哈希表能达到O(1)的查询速率，同时还能去除数组中的冗余元素；
2.主要能优化的部分为查找连续序列中子序列的重复计算，以x为起始点的序列，确保x为序列端点可以通过哈希表中不含x-1实现。这样，各连续序列都只需遍历一次，为最优方案；
3.!以上解法中，构建的哈希表起到了并查集的作用，后续的解题思路可以参考这种做法。
...
(字节T4)给定二维平面整数点集，若图中任意点都不在x的右上方(即横纵坐标不同时大于x)，称x为最大的，求出所有的最大点。


(字节T5)*给定一整数(<=100)序列，要求求出一个区间，使得区间中最小数*区间所有数之和的值最大。返回该最大值。


(852)PeakIndexInMountainArray/山脉数组峰顶索引 -- E
给定一山脉数组，寻找峰值索引。


(162)findPeak/寻找峰值索引 -- M
给定一含峰数组，返回其最后一个峰值索引。
...
1.单向链表只有一个指向下一节点的指针；
优点：增删节点简单；
缺点：只有顺序遍历方向，不饿能找到前驱点；
适用场景：含有大量节点增删的情况。
2.双向链表有一对前后指针；
优点：可以前后索引；
缺点：增删节点步骤稍微多些；
适用场景：适用于需要大量双向查找的情况。
3.使用伪头部和伪尾部节点可以标定链表的界限，同时在不确定链表头伪的情况下提供预先指针。
...
>Summary:
1.判断环是否存在使用快慢指针即可
2.判断环的起点用Floyd判环法。注意判环的关系由距离（步数）决定，而非节点数，所以快慢指针需要在链头/节点0开始，而不是初始化在0，1节点。
这里节点从0开始计数而非1其实蕴含有拓扑知识，起点在线上，点的数量比线要多1；若起点在环上，点的数量和线的数量相同，点0其实就是点N。所以在统计距离的背景下，起始节点为0，不计入距离.
...
>Summary:
1.快慢指针法查找链表中点；
2.node.next=None 切断链表；
3.*链表切分合并操作对应的指针操作(往往有多少个特定作用的对象就需要对应数目的指针记录节点位置)；
...
给定n个非负整数，表示并排的柱形高度，柱子宽度为1，求该柱形图中最大的矩形面积。
1.单调栈；
2.类似的单调栈问题42、739、496、316、901、402、581


(42)接雨水
给定一数组表示石柱台面，宽度为1，问最多能接多少雨水。
>Summary:
1.
2.
3.
...
(字节T5)*给定一整数(<=100)序列，要求求出一个区间，使得区间中最小数*区间所有数之和的值最大。返回最大值。
>Summary:
...
(114)!flatten
将二叉树按中序$原地$展开为单链表（原地指不开辟新内存存储对象内容，直接修改原有数据结构的内容）
>Summary:
$递归，迭代实现，莫里斯遍历$
1.左右子树的遍历顺序有明显的后进（遍历左）先出（遍历右）的结构。故可用栈存储待回归的节点 + 一层循环负责深度遍历另一层循环负责切换子树(宽度遍历)实现迭代遍历；
2.若要同步展开需要保存左右子树信息，将右节点、左节点按顺序入栈展开；使用递归会复杂一些，因为递归是隐性地使用栈，递归完成前并不知道二叉树的规模，同步合成右子树需要查找前驱节点；
3.*莫里斯遍历，查找前驱节点的方式是
...
(124)max_path_sum
给定一非空二叉树，返回其最大路径和。路径为从任意节点出发，到达任意节点的序列，至少包含一个节点。
>Summary:
1.由于至少需要包含一个节点，所以一个子树结构中有(左，右，根，左路径+根，右路径+根，左路径+右路径+根)六种选法。可见根节点的值一定要在更新式里面，左右路径则是＞0才添加到更新式里；遇见叶结点则直接返回节点值；
2.含负数场景，和值初始化为MIN_VALUE;
...
(208)Trie/实现前缀树
实现一个前缀树，包含insert,search,startsWith三个操作。（假设所有字母为小写字母，没有空字符输入）
>Summary:
0.相似题型：
①添加搜索单词 -- 211
②单词替换 -- 648
③实现魔法字典 -- 676
④设计搜索自动补全系统
1.前缀树应该使用链表数组实现（字母个数固定），节点位于数组的位置代表字母键值。有containsKey(key),get(key),put(key,node),setEnd(),isEnd()等内置方法。End对象变量代表节点是否为单词结尾。
...
##2## 环形存在判断
1.快慢指针判环
设定快慢指针，快指针速度为2，慢指针速度为1.若存在环，每轮两指针的距离减一，总会相遇。  
...
(1192)！criticalConnection
给定服务器集群网络，查找其中的关键连接（断开后导致某些服务器连接不到其他服务器的连接），即不可替代的连接。
>Summary:
1.很好的DFS状态设计题目


##5## 拓扑排序
拓扑排序是一个有向无环图，适合用于处理有依赖关系事务的排序。图中的节点只出现一次，若有A->B，表明A排在B前。每一次遍历将入度为0(没有其他前驱点)的顶点推出。
拓扑排序可以通过维护一个入度为0的顶点集合来实现
...
>Summary:
1.考察拓扑排序，即连接节点的出现顺序；DAG中确认拓扑排序或是否有环的问题最优复杂度为O(V+E)；
2.先统计节点的入度数以及邻接表，使用贪心和BFS重复删除入度为0的节点，若最后仍然有入度非0的节点留下，说明存在环；
3.DFS的方法也能达到最佳复杂度，在记录已访问点的方法里，维护状态数组比维护已访问哈希表要更加便捷。因为哈希集存储已访问节点的做法只有两种状态，需要在搜索和回溯的过程中手动维护；而状态数组可以保存节点的所有状态：①未访问；②访问中；③访问完，只要遍历的子节点中出现访问中状态的点，就能说明存在环。


##6## 岛屿问题
(商汤T3)统计大陆面积
给定M×N的01地图，0表示水，1表示陆地。规定最下方的边为陆地，以此展开的四邻域为陆地。被陆地围住的水是噪声，判为陆地，问陆地面积多少。(被水围住的岛不算)
>Summary:
1.用一个递归函数可以实现连通域的值更改以及统计操作。只需要对上左右的海遍历出来并做标记，将非标记的海改为陆地。最后通过底边的任意一点开始连通域统计就能实现大陆的面积统计。


(463)岛屿的周长
给定二维地图，1表示陆地0表示水域。水陆在水平和垂直方向相连，假设岛屿没有湖，只有一个岛屿，求岛屿周长。
>Summary:
1.直接遍历


(695)岛屿最大面积
给定01grid，1表示陆地，水平垂直连接的陆地才算相邻。要求求出最大岛屿的面积。
...
# ---------------------------------------------
>Summary:
1.构建查询的一般思路：①优先考虑单表，group by,order by,limit等效率也比多表高；②其次考虑用连接，小表驱动大表+建立合适索引+合理运用连接条件能解决大部分问题。但join级数不宜过多；③子查询、笛卡尔积尽量不要使用，因为每条记录都要操作一遍，效率过低；④自定义变量在复杂SQL中用处很大；⑤对于带聚合功能的查询，应用窗口函数往往是最优选择;
2.不走索引的语句：①NOT IN
3.NULL：①NULL代表不确定，不是“空”、“无”的意思，判断NULL只能用IS NULL、IS NOT NULL；=NULL除了在初始分配语句，其他地方使用都是不对的；
4.SELECT * 和SELECT 1不同，后者存在记录就返回1；前者搜索所有变量，会影响索引的命中；
...
## 连接
>Summary:
1.MySQL 关联算法为Nested Loop Join，查询时注意使用小表驱动大表，减少外层循环次数；
...
## 排序
>Summary:
1.场景：①重复项连续排名（类似于编号）；②重复项跳级排名；③不计重复项连续排名。
2.常用方法：单表查询；子查询；自连接；笛卡尔积；自定义变量；窗口函数；
3.可用内置函数：①row_number() 考虑重复，同值不同名；②rank() 考虑重复，同值同名，有跳级；③dense_rank() 不考虑重复，同值同名，连续；
以上函数都要跟over() 搭配使用，其参数为partition by和 order by.


# 不考虑重复，同值同名
(S176)第二高薪水
>Summary:
1.LIMIT offset, n / LIMIT n, OFFSET offset
*2.子查询虚表没有记录为什么会返回NULL?


(S177)第N高薪水 ▲
>Summary:
1.LIMIT 和OFFSET 只接受正整数或单一变量，0、负数、小数和表达式都不行；
2.*函数、变量的运用
3.自定义函数的一般写法：①定义变量接收返回值；②执行查询条件，并赋值相应的变量；③返回结果


(S178)分数排名
>Summary:
1.新增统计字段
2.rank 是MYSQL关键字，需要加''转义使用
3.MySQL中','的作用
...
(S185)查找各部门薪资前三高的员工
>Summary:
1.难点：分组排名，且存在部分组别不足N个的情况；
2.!使用自连接 + count(DISTINCT t2.var) < N选出前N项，并通过连接实现分组；
...
(S182)查找重复的电子邮箱
>Summary:
1.Group By 除了使用临时表+WHERE实现条件查询，更常用的是+HAVING子句添加条件；
...
(S184)部门工资最高的员工
>Summary:
1.建立好分组元信息临时表后，再通过IN选择符合条件的记录;
2.多列字段条件查询，列的顺序要和子查询中列的顺序保持一致；
3.另一种方法是使用多表内连接，但效率不高；
